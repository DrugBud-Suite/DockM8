...
from scripts.pose_selection import *
...
parser.add_argument(
    ...
    type=str,
    default="single",
    choices=["Single", "Ensemble", "active_learning"],
    help="Specifies the mode: single, ensemble, or active_learning",
)

parser.add_argument(
    "--gen_decoys", default=False, type=str2bool, help="Whether or not to generate decoys using DeepCoy"
)
parser.add_argument(
    "--decoy_model",
    default="DUDE",
...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    help="Path to the protein file(s) or protein files if using ensemble docking mode",
)
...
parser.add_argument(
    ...
    default=True,
    type=str2bool,
    help="Whether or not to add hydrogens to the protein using Protoss (True for yes, False for no)",
)
parser.add_argument(
    "--conformers",
    default="RDKit",
    ...
    help="Method to use for conformer generation (RDKit and MMFF are equivalent)",
)
parser.add_argument(
    "--protonation",
    required=True,
...
parser.add_argument(
    ...
    default=False,
    type=str2bool,
    help="Whether or not to remove problematic poses with PoseBusters (True for yes, False for no)",
)
parser.add_argument(
    "--pose_selection",
    required=True,
...
parser.add_argument("--exhaustiveness", default=8, type=int, help="Precision of SMINA/GNINA/QVINAW/QVINA2")
parser.add_argument("--ncpus", default=int(os.cpu_count() * 0.9), type=int, help="Number of CPUs to use")
parser.add_argument(
    "--clustering_method", type=str, choices=["KMedoids", "Aff_Prop", "None"], help="Clustering method to use"
)
parser.add_argument(
    "--rescoring", type=str, nargs="+", choices=list(RESCORING_FUNCTIONS.keys()), help="Rescoring methods to use"
)
parser.add_argument(
    "--consensus", type=str, required=True, choices=list(CONSENSUS_METHODS.keys()), help="Consensus method to use"
)
...
if any(metric in args.pose_selection for metric in CLUSTERING_METRICS.keys()) and (
    args.clustering_method == None or args.clustering_method == "None"
):
    parser.error(
        "Must specify a clustering method when --pose_selection is set to 'RMSD', 'spyRMSD', 'espsim' or 'USRCAT'"
    )
...
def dockm8(
    ...
    if pocket == "Reference":
        ...
        pocket_definition = binding_site_coordinates_dogsitescorer(prepared_receptor, w_dir, method=dogsitescorer_mode)
    else:
    ...
    for method in pose_selection:
        if not os.path.isfile(w_dir / f"clustering/{method}_clustered.sdf"):
            select_poses(
                method, clustering_method, w_dir, prepared_receptor, pocket_definition, software, all_poses, ncpus
            )
...
def run_command(**kwargs):
    ...
    if kwargs.get("gen_decoys") == True:
        if kwargs.get("mode") == "Single":
            ...
            # Generate decoys
            output_library = generate_decoys(
                Path(kwargs.get("actives")), kwargs.get("n_decoys"), kwargs.get("decoy_model"), kwargs.get("software")
            )
            ...
            performance = calculate_performance(output_library.parent, output_library, [10, 5, 2, 1, 0.5])
        ...
        if kwargs.get("mode") == "Ensemble":
            ...
            ensemble_consensus(
                receptors, optimal_conditions["clustering"], optimal_conditions["consensus"], kwargs.get("threshold")
            )
    else:
        ...
        if kwargs.get("mode") == "Ensemble":
            ...
            ensemble_consensus(
                receptors, kwargs.get("pose_selection"), kwargs.get("consensus"), kwargs.get("threshold")
            )
