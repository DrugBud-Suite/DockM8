...
parser.add_argument('--exhaustiveness',
                    default=8,
                    type=int,
                    help='Precision of SMINA/GNINA/QVINAW/QVINA2')
...
parser.add_argument('--clustering_method',
                    type=str,
                    choices=['KMedoids', 'Aff_Prop', 'None'],
                    help='Clustering method to use')
parser.add_argument('--rescoring',
                    type=str,
                    nargs='+',
...
if (args.pocket == 'Reference' or args.pocket == 'RoG') and not args.reffile:
    parser.error(
        f"Must specify a reference ligand file when --pocket is set to {args.pocket}"
    )

if any(metric in args.pose_selection
       for metric in CLUSTERING_METRICS.keys()) and (
...
if args.gen_decoys == True and not args.n_decoys:
    parser.error(
        "Must specify the number of decoys to generate when --gen_decoys is set to True"
    )

if args.gen_decoys == True and not args.actives:
    parser.error(
        "Must specify the path to the actives file when --gen_decoys is set to True"
    )
...
for program in DOCKING_PROGRAMS:
    if f"bestpose_{program}" in args.pose_selection and program not in args.docking_programs:
        parser.error(
            f"Must specify {program} in --docking_programs when --pose_selection is set to bestpose_{program}"
        )
...
def run_command(**kwargs):
    ...
    if kwargs.get('gen_decoys') == True:
        if kwargs.get('mode') == 'Single':
            ...
            # Generate decoys
            output_library = generate_decoys(Path(kwargs.get('actives')),
                                             kwargs.get('n_decoys'),
                                             kwargs.get('decoy_model'),
                                             kwargs.get('software'))
            ...
            if '_' in optimal_conditions['clustering']:
                ...
                docking_programs = kwargs.get('docking_programs')
            optimal_rescoring_functions = list(
                optimal_conditions['scoring'].split('_'))
            # Save optimal conditions to a file
            with open((kwargs.get('receptor'))[0].parent / 'DeepCoy' /
        ...
        if kwargs.get('mode') == 'Ensemble':
            ...
            for receptor, ref in receptor_dict.items():
                dockm8(software=Path(kwargs.get('software')),
                       ...
                       rescoring=optimal_rescoring_functions,
                       consensus=optimal_conditions['consensus'])
            ensemble_consensus(receptors, optimal_conditions['clustering'],
                               optimal_conditions['consensus'],
        ...
        if kwargs.get('mode') == 'Ensemble':
            ...
            for receptor, ref in receptor_dict.items():
                dockm8(software=Path(kwargs.get('software')),
                       ...
                       rescoring=kwargs.get('rescoring'),
                       consensus=kwargs.get('consensus'))
            ensemble_consensus(receptors, kwargs.get('pose_selection'),
                               kwargs.get('consensus'),
                               kwargs.get('threshold'))
