        ...
        if col not in ['Pose ID', 'ID']:
            ...
            if function_info:
                if standardization_type == 'min_max':
                    ...
                    col_min, col_max = np.percentile(
                        column_data, [1, 99]) if function_info[
                            'best_value'] == 'max' else np.percentile(
                                column_data, [99, 1])
                    df[col] = min_max_standardization(
        ...
        for consensus_method in consensus_methods:
            ...
            if conensus_type == 'rank':
                consensus_dataframe = conensus_function(
                    ranked_dataframe, selection_method, [
                        col for col in ranked_dataframe.columns
                        if col not in ['Pose ID', 'ID']
                    ])
            elif conensus_type == 'score':
                consensus_dataframe = conensus_function(
                    standardized_dataframe, selection_method, [
            ...
            if selection_method in [
                ...
                poses = PandasTools.LoadSDF(str(
                    w_dir / 'clustering' / f'{selection_method}_clustered.sdf'),
                                            molColName='Molecule',
                                            idName='Pose ID')
