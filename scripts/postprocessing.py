...
def standardize_scores(df: pd.DataFrame, standardization_type: str):
    ...
    def min_max_standardization(score, best_value, min_value, max_value):
        ...
        return (
            ...
            if best_value == "max"
            else (max_value - score) / (max_value - min_value)
        )
            ...
            if function_info:
                if standardization_type == "min_max":
                    ...
                    df[col] = min_max_standardization(
                        df[col], function_info["best_value"], df[col].min(), df[col].max()
                    )
                    ...
                    col_min, col_max = (
                        np.percentile(column_data, [1, 99])
                        if function_info["best_value"] == "max"
                        else np.percentile(column_data, [99, 1])
                    )
...
def rank_scores(df):
    ...
    """
    df = df.assign(
        **{col: df[col].rank(method="average", ascending=False) for col in df.columns if col not in ["Pose ID", "ID"]}
    )
    return df


def apply_consensus_methods(
    w_dir: str, selection_method: str, consensus_methods: str, rescoring_functions: list, standardization_type: str
):
    """
        ...
        for consensus_method in consensus_methods:
            ...
            if conensus_type == "rank":
                consensus_dataframe = conensus_function(
                    ranked_dataframe,
                    selection_method,
                    [col for col in ranked_dataframe.columns if col not in ["Pose ID", "ID"]],
                )
            elif conensus_type == "score":
                consensus_dataframe = conensus_function(
                    standardized_dataframe,
                    selection_method,
                    [col for col in standardized_dataframe.columns if col not in ["Pose ID", "ID"]],
                )
            else:
            ...
            if selection_method in [
                ...
                poses = PandasTools.LoadSDF(
                    str(w_dir / "clustering" / f"{selection_method}_clustered.sdf"),
                    molColName="Molecule",
                    idName="Pose ID",
                )
                ...
                consensus_dataframe.to_csv(
                    Path(w_dir) / "consensus" / f"{selection_method}_{consensus_method}_results.csv", index=False
                )
            else:
                consensus_dataframe.to_csv(
                    Path(w_dir) / "consensus" / f"{selection_method}_{consensus_method}_results.csv", index=False
