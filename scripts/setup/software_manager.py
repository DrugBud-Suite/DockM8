...
SOFTWARE_INFO: Dict[str, Dict[str, Callable]] = {
    "GNINA": {"install": install_gnina, "check": lambda path: (path / "gnina").is_file()},
    "QVINAW": {"install": install_qvina_w, "check": lambda path: (path / "qvina-w").is_file()},
    "QVINA2": {"install": install_qvina2, "check": lambda path: (path / "qvina2.1").is_file()},
    "PSOVINA": {"install": install_psovina, "check": lambda path: (path / "psovina").is_file()},
    "PLANTS": {"install": install_plants, "check": lambda path: (path / "PLANTS").is_file()},
    "FABind": {"install": install_fabind, "check": lambda path: (path / "FABind").is_dir()},
    "PANTHER": {"install": install_panther, "check": lambda path: (path / "panther").is_dir()},
    "PLANTAIN": {"install": install_plantain, "check": lambda path: (path / "plantain").is_dir()},
    "AA_SCORE": {"install": install_aa_score, "check": lambda path: (path / "AA-Score-Tool-main").is_dir()},
    "CONVEX_PLR": {"install": install_convex_pl, "check": lambda path: (path / "Convex_PL").is_file()},
    "CENSIBLE": {"install": install_censible, "check": lambda path: (path / "censible").is_file()},
    "DLIGAND2": {"install": install_dligand2, "check": lambda path: (path / "DLIGAND2").is_file()},
    "LIN_F9": {"install": install_lin_f9, "check": lambda path: (path / "LinF9").is_file()},
    "GYPSUM_DL": {"install": install_gypsum_dl, "check": lambda path: (path / "gypsum_dl-1.2.1").is_dir()},
    "SCORCH": {"install": install_scorch, "check": lambda path: (path / "SCORCH-1.0.0").is_dir()},
    "RF_SCORE_VS": {"install": install_rf_score_vs, "check": lambda path: (path / "rf-score-vs").is_file()},
    "RTMSCORE": {"install": install_rtmscore, "check": lambda path: (path / "RTMScore-main").is_file()},
    "IT_SCORE_AFF": {"install": install_itscoreAff, "check": lambda path: (path / "ITScoreAff_v1.0").is_dir()},
    "KORP_PL": {"install": install_korp_pl, "check": lambda path: (path / "KORP-PL").is_file()},
    "POSECHECK": {"install": install_posecheck, "check": lambda path: (path / "posecheck-main").is_file()},
    "GENSCORE": {"install": install_genscore, "check": lambda path: (path / "GenScore").is_dir()},
    "MGLTOOLS": {"install": install_mgltools, "check": check_mgltools}
}
