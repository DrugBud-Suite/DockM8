...
def calculate_ccr(scores: np.ndarray, labels: np.ndarray, percentile: float | list[float]) -> float | list[float]:
    """
    Calculate Correct Classification Rate (CCR) for virtual screening results.
    ...
    for p in percentiles:
        ...
        if not n == 0 and not N - n == 0:
            ccr = 0.5 * ((ns / n) + ((N - Ns - n + ns) / (N - n)))

        results.append(round(ccr, 3))
