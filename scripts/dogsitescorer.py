...
def upload_pdb_file(filepath: Path):
    ...
    protein_id = _send_request_get_results(
        "get", [APIConsts.FileUpload.RESPONSE_MSG_FETCH_ID["id"]],
        url_of_id)[0]
    return protein_id
    ...
    r = requests.post(
        ...
        json={
            "dogsite": {
                ...
                "analysisDetail": "0",  # 1 = include subpockets in results
...
def calculate_pocket_coordinates_from_pocket_pdb_file(filepath):
    ...
    coordinates = [
        float(element) for element in coordinates_data_as_list
...
def binding_site_coordinates_dogsitescorer(pdbpath: Path,
                                           w_dir: Path,
                                           method='Volume'):
    """
	...
	"""
    printlog("Finding Binding Sites with DogSiteScorer...")
    # Upload the PDB file
