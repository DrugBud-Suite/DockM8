...
def parallel_executor(
    function, list_of_objects: list, n_cpus: int, job_manager="concurrent_process", display_name: str = None, **kwargs
):
    """
    ...
    if job_manager == "concurrent_process":
        ...
        results = Parallel(n_jobs=n_cpus)(
            tqdm(jobs, total=len(list_of_objects), desc=f"Running {display_name if display_name else function_name}")
        )
    else:
