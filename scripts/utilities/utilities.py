...
def delete_files(folder_path: str, save_file: str | list[str]) -> None:
    """
...
def parallel_SDF_loader(
    sdf_path: Path, molColName: str, idName: str, n_cpus: int | None = None, SMILES: str | None = None
) -> pd.DataFrame:
    """
...
def get_sdf_length(sdf_path: str | Path) -> int:
    """
    Get the number of complete compounds in an SDF file.
    
    Args:
        sdf_path (str | Path): Path to the SDF file
        
    Returns:
        int: Number of complete compounds (count of '$$$$' delimiters)
    ...
    return sdf_path.read_text().count('$$$$')
