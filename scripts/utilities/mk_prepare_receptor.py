...
def get_args():
    ...
    parser.add_argument(
        ...
        help="project center of grid box along CA-CB bond 5 A away from CB",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--r_eq_13_scaling", default=0.5, type=float, help="r_eq scaling for 1-3 interaction across reactive atoms"
    )
    parser.add_argument(
        "--r_eq_14_scaling", default=0.5, type=float, help="r_eq scaling for 1-4 interaction across reactive atoms"
    )
...
for react_name_str in args.reactive_name:
    ...
    if ok:
        if resname in modified_resnames:
            print(
                "Command line error: repeated resname %s passed to --reactive_resname" % resname + os_linesep,
                file=sys.stderr,
            )
            sys.exit(2)
...
any_lig_base_types = ["HD", "C", "A", "N", "NA", "OA", "F", "P", "SA", "S", "Cl", "CL", "Br", "BR", "I", "Si", "B"]
...
if not args.skip_gpf:
    ...
    gpf_string, npts = gridbox.get_gpf_string(
        box_center, box_size, rigid_fn, rec_types, any_lig_base_types, ff_param_fname=ff_fn.name
    )
    # write GPF
    ...
    for atom in receptor.atoms(pdbqt["flex_indices"]):
        if gridbox.is_point_outside_box(atom["xyz"], box_center, npts):
            ...
            print(
                "WARNING: Strongly recommended to use a box that encompasses flexible residues." + os_linesep,
                file=sys.stderr,
            )
...
if len(reactive_flexres) > 0:
    ...
    derivtypes, modpairs, collisions = get_reactive_config(
        any_lig_reac_types, rec_reac_types, args.eps_12, args.r_eq_12, args.r_eq_13_scaling, args.r_eq_14_scaling
    )
    ...
    if len(collisions) > 0:
        ...
        written_files_log["description"].append(
            "type pairs (n=%d) that may lead to intra-molecular reactions" % len(collisions)
        )
