...
def standardize_scores(df: pd.DataFrame, standardization_type: str):
	...
	def min_max_standardization(score, best_value, min_value, max_value):
		...
		"""
		return ((score-min_value) / (max_value-min_value) if best_value == "max" else
	...
	for col in df.columns:
		if col not in ["Pose ID", "ID"]:
			...
			scoring_function = RESCORING_FUNCTIONS.get(col)
			if scoring_function:
				if standardization_type == "min_max":
					...
					col_min, col_max = (np.percentile(column_data, [1, 99])
							if scoring_function.best_value == "max"
							else np.percentile(column_data, [99, 1]))
					df[col] = min_max_standardization(df[col], scoring_function.best_value, col_min, col_max)
				else:
