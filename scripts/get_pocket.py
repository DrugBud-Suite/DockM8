...
def calculate_min_distance_protein(protein_dataframe, ligand_molecule):
    ...
    ligand_coordinates = add_coordinates(
        get_ligand_coordinates(ligand_molecule))
    protein_dataframe['min_dist'] = protein_dataframe.apply(
        lambda row: calculate_min_distance(
            row['coordinates'], ligand_coordinates), axis=1)
...
def select_cutoff_residues(protein_dataframe, ligand_molecule, cutoff):
    ...
    selected_residues = list(
        set(list(residues_within_cutoff['chain_residue_id'])))
    protein_dataframe = protein_dataframe[protein_dataframe['chain_residue_id'].isin(
        selected_residues)]
