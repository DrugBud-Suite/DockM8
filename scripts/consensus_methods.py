...
def ECR_best(df: pd.DataFrame, clustering_metric: str,
    ...
    df.sort_values(f'ECR_best_{clustering_metric}',
                   ascending=False,
                   inplace=True)
...
def ECR_avg(df: pd.DataFrame, clustering_metric: str,
            selected_columns: list) -> pd.DataFrame:
    """
...
def avg_ECR(df: pd.DataFrame, clustering_metric: str,
            selected_columns: list) -> pd.DataFrame:
    """
    ...
    mean_ranks = df.groupby('ID',
                            as_index=False)[selected_columns].mean().round(2)
    ...
    output_df = pd.DataFrame({
        'ID': mean_ranks['ID'],
        f'avg_ECR_{clustering_metric}': ecr_sum
    })

    return output_df[['ID', f'avg_ECR_{clustering_metric}']]


def RbR(df: pd.DataFrame, clustering_metric: str,
        selected_columns: list) -> pd.DataFrame:
    """
...
def RbV(df: pd.DataFrame, clustering_metric: str,
        selected_columns: list) -> pd.DataFrame:
    """
    ...
    for column in selected_columns:
        df[f'RbV_{clustering_metric}'] += (
            df[column] > df[column].quantile(0.95)).astype(int)
...
def Zscore_best(df: pd.DataFrame, clustering_metric: str,
                selected_columns: list) -> pd.DataFrame:
    '''
    ...
    # Calculate Z-scores
    z_scores = (df[selected_columns] -
                df[selected_columns].mean()) / df[selected_columns].std()
    ...
    df = df.sort_values(f'Zscore_best_{clustering_metric}',
                        ascending=False).drop_duplicates('ID')

    return df[['ID', f'Zscore_best_{clustering_metric}']]


def Zscore_avg(df: pd.DataFrame, clustering_metric: str,
               selected_columns: list) -> pd.DataFrame:
    """
    Calculates the Z-score consensus scores for each row in the given DataFrame,
