...
class LinF9(ScoringFunction):
    ...
    def rescore(self, sdf_file: str, n_cpus: int, protein_file: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            return self._combine_rescoring_results(linf9_dataframes)

        except Exception as e:
            printlog("ERROR: An unexpected error occurred during LinF9 rescoring:")
            printlog(str(e))
            return pd.DataFrame()
        finally:
            self.cleanup()
    ...
    def _rescore_split_file(self, split_file: Path, protein_file: str) -> Path | None:
        ...
        stdout, stderr = run_subprocess_command(command=linf9_cmd)
        
        if not results.exists():
            printlog(f"LinF9 output file not found: {results}")
            if stderr:
                printlog(f"LinF9 command output:\n{stdout}")
                printlog(f"LinF9 command error output:\n{stderr}")
            return None
