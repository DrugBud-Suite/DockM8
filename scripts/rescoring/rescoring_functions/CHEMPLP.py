...
class CHEMPLP(ScoringFunction):
	...
	def rescore(self, sdf: str, n_cpus: int, **kwargs) -> DataFrame:
		...
		ants = "20"
		chemplp_rescoring_folder = Path(rescoring_folder) / f"{self.column_name}_rescoring"
		...
		chemplp_config = [
			...
			"# output\n",
			f"output_dir {chemplp_rescoring_folder / 'results'}\n",
		...
		chemplp_rescoring_results.to_csv(rescoring_folder / f"{self.column_name}_rescoring" /
