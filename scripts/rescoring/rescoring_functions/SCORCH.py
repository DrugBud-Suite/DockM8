...
class SCORCH(ScoringFunction):
	...
	def rescore(self, sdf_file: str, n_cpus: int, protein_file: str, **kwargs) -> pd.DataFrame:
		...
		try:
			...
			printlog(f"Rescoring with SCORCH complete in {end_time - start_time:.4f} seconds!")
			return scorch_rescoring_results
		except Exception as e:
			printlog(f"ERROR: An unexpected error occurred during SCORCH rescoring:")
			printlog(traceback.format_exc())
			return pd.DataFrame()
		finally:
			self.remove_temp_dir(temp_dir)

	def _rescore_split_file(self, split_file: Path, scorch_protein: Path) -> Path:
		"""
		...
		try:
			...
			scorch_cmd = (f"{self.software_path}/SCORCH-1.0.0/scorch.py"
				...
				f" --out {results}"
				" --return_pose_scores")
			subprocess.run(scorch_cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
		except Exception as e:
	...
	def _combine_rescoring_results(self, dataframes: List[pd.DataFrame]) -> pd.DataFrame:
		...
		try:
			...
			combined_results.rename(columns={"SCORCH_pose_score": self.column_name}, inplace=True)
