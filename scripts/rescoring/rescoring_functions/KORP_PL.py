...
class KORPL(ScoringFunction):
	...
	def rescore(self, sdf_file: str, n_cpus: int, protein_file: str, **kwargs) -> pd.DataFrame:
		...
		try:
			...
			printlog(f"Rescoring with KORP-PL complete in {end_time - start_time:.4f} seconds!")
			return korpl_rescoring_results
		except Exception as e:
			printlog(f"ERROR: An unexpected error occurred during KORP-PL rescoring:")
	...
	def _rescore_split_file(self, split_file: Path, protein_file: str) -> Path:
		...
		try:
			...
			result = subprocess.run(korpl_cmd, shell=True, capture_output=True, text=True, check=True)
			...
			printlog(f"KORP-PL rescoring failed for {split_file}:")
			printlog(traceback.format_exc())
			return None
