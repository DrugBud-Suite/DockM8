...
class PLP(ScoringFunction):
    ...
    def _rescore_split_file(self, split_file: Path, pocket_definition: dict) -> Path | None:
        ...
        try:
            ...
            plp_cmd = f"{self.executable_path} --mode rescore {str(config_file)}"
            stdout, stderr = run_subprocess_command(command=plp_cmd)

            if not results_csv.exists():
                printlog(f"PLP output file not found for {split_file}")
                if stderr:
            ...
            plp_results[["Pose ID", self.column_name]].to_csv(output_csv, index=False)
    ...
    def _combine_rescoring_results(self, result_files: list[Path]) -> pd.DataFrame:
        ...
        try:
            ...
            if not dataframes:
                printlog("No valid CSV files found with PLP scores.")
                return pd.DataFrame()
