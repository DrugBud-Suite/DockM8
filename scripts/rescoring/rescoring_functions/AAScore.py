# =============================================================================
# AAScore scoring function - REMOVED FROM PUBLICATION
...
#         """
#         try:
#             pocket_file = protein_file.with_name(protein_file.stem + "_pocket.pdb")
#
#             if n_cpus == 1:
...
#             self.cleanup()
#
#     def _rescore_single_process(self, sdf_file: Path, pocket_file: Path, temp_dir: Path) -> pd.DataFrame:
#         results = temp_dir / "rescored_AAScore.csv"
#         aascore_cmd = (
#             f"cd {str(self.software_path)}/AA-Score-Tool-main &&"
#             f" conda run -n AAScore python AA_Score.py"
#             f" --Rec {str(pocket_file)}"
#             f" --Lig {str(sdf_file)}"
#             f" --Out {str(results)}"
#         )
#
...
#             return pd.DataFrame()
#
#     def _rescore_multi_process(self, sdf_file: Path, pocket_file: Path, n_cpus: int, temp_dir: Path) -> pd.DataFrame:
#         """
#         Rescore using multiple processes.
#
#         Args:
#             sdf_file (Path): The path to the SDF file.
#             pocket_file (Path): The path to the pocket file.
#             n_cpus (int): The number of CPUs to use for parallel processing.
...
#         """
#         split_files_folder = split_sdf(str(sdf_file), self._temp_dir, mode="cpu", splits=n_cpus)
