...
class NNScore(ScoringFunction):
	...
	def __init__(self, software_path: Path):
		super().__init__("NNScore", "NNScore", "max", (0, 20), software_path)
	...
	def rescore(self, sdf_file: str, n_cpus: int, protein_file: str, **kwargs) -> pd.DataFrame:
		...
		try:
			pickle_path = self.software_path / "models/NNScore_pdbbind2016.pickle"
			results = temp_dir / "rescored_NNscore.sdf"

			nnscore_cmd = (f"oddt_cli {sdf_file}"
			...
			try:
				...
				printlog(f"ERROR: NNScore rescoring failed:")
			...
			printlog(f"Rescoring with NNScore complete in {end_time - start_time:.4f} seconds!")
			return nnscore_results_df
		except Exception as e:
			printlog(f"ERROR: An unexpected error occurred during NNScore rescoring:")
			printlog(traceback.format_exc())
			return pd.DataFrame()
		finally:
