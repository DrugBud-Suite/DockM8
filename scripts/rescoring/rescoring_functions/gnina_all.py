...
class GninaAll(ScoringFunction):
    """GNINA-All scoring function implementation."""

    def __init__(self, software_path: Path):
        # Special handling for "all" score type
        super().__init__(
            name="GNINA-All",
            column_name="GNINA-All",  # Special multi-column handling in rescore method
            best_value="min",  # Using first value for initialization
            score_range=(100, -100),  # Using first range for initialization
            software_path=software_path,
        )
        # Store all column names for later use
        self.column_names = ["GNINA-Affinity", "CNN-Score", "CNN-Affinity"]
    ...
    def rescore(self, sdf_file: Path, n_cpus: int, protein_file: Path, **kwargs) -> pd.DataFrame:
        """
        Rescore the molecules in the given SDF file using the GNINA-All scoring function.
    ...
    def _combine_rescoring_results(self, dataframes: list[pd.DataFrame]) -> pd.DataFrame:
        ...
        try:
            ...
            # For "all" score type, return all three columns
            return combined_results[["Pose ID", "GNINA-Affinity", "CNN-Score", "CNN-Affinity"]]
