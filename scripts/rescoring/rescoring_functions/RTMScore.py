...
class RTMScore(ScoringFunction):
	...
	def __init__(self):
		super().__init__("RTMScore", "RTMScore", "max", (0, 100))
	...
	def rescore(self, sdf: str, n_cpus: int, **kwargs) -> DataFrame:
		...
		RTMScore_rescoring_results = str(rescoring_folder / f"{self.column_name}_rescoring" /
											f"{self.column_name}_scores.csv")
		try:
			RTMScore_command = (
				f'cd {rescoring_folder / "RTMScore_rescoring"} && python {software}/RTMScore-main/example/rtmscore.py' +
				f' -p {str(protein_file).replace(".pdb", "_pocket.pdb")}' + f" -l {sdf}" + " -o RTMScore_scores" +
				" -pl"
