...
def RTMScore_rescoring(sdf: str, n_cpus: int, column_name: str, **kwargs):
	...
	RTMScore_rescoring_results = str(rescoring_folder / f"{column_name}_rescoring" / f"{column_name}_scores.csv")
	try:
		RTMScore_command = (
			f'cd {rescoring_folder / "RTMScore_rescoring"} && python {software}/RTMScore-main/example/rtmscore.py' +
			f' -p {str(protein_file).replace(".pdb", "_pocket.pdb")}' + f" -l {sdf}" + " -o RTMScore_scores" + " -pl"
	...
	def RTMScore_rescoring_splitted(split_file, protein_file):
		RTMScore_command = (
			f'cd {rescoring_folder / "RTMScore_rescoring"} && python {software}/RTMScore-main/example/rtmscore.py' +
			f' -p {str(protein_file).replace(".pdb", "_pocket.pdb")}' + f" -l {split_file}" +
	...
	for file in tqdm(split_files_sdfs):
		try:
			RTMScore_command = (
				f'cd {rescoring_folder / "RTMScore_rescoring"} && python {software}/RTMScore-main/example/rtmscore.py' +
				f' -p {str(protein_file).replace(".pdb", "_pocket.pdb")}' + f" -l {file}" +
				f" -o RTMScore_scores_{file.stem}" + " -pl"
				f" -m {software}/RTMScore-main/trained_models/rtmscore_model1.pth")
			subprocess.call(RTMScore_command, shell=True, stdout=DEVNULL, stderr=STDOUT)
		except Exception as e:
