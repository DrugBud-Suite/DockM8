...
def rescore_all(
    ...
    for function in functions:
        # Check if the function exists in the rescoring_functions dictionary
        if function in rescoring_functions:
            # Check if the scores.csv file exists
            score_file_path = rescoring_folder / f'{function}_rescoring' / f'{function}_scores.csv'
            if not score_file_path.is_file():
                # Call the selected function
                selected_function = rescoring_functions[function]['function_name']
                selected_function(clustered_sdf, ncpus, **parameters)
            else:
                skipped_functions.append(function)
        else:
            # Function not found in the JSON file
