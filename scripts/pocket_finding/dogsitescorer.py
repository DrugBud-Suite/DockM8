...
def _send_request_get_results(
    request_type: str, keys_list: List[str], url: str, task: str = "Fetching results from DoGSiteScorer API", **kwargs
) -> List[Union[str, Dict]]:
    """
...
def calculate_pocket_coordinates_from_pocket_pdb_file(filepath: Union[str, Path]) -> Dict[str, List[float]]:
    ...
    pocket_coordinates = {"center": coordinates[:3], "size": [coordinates[-1] for dim in range(3)]}
