...
def find_pocket_p2rank(software: Path, receptor: Path, radius: int):
	...
	p2rank_command = f'{software / "p2rank" / "prank"} predict -f {receptor} -o {output_dir}'
	subprocess.run(p2rank_command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
