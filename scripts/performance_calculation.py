...
def process_combination(combination, clustering_method, ranked_df,
                        standardised_df, actives_df, percentages):
    ...
    for method in CONSENSUS_METHODS.keys():
        if CONSENSUS_METHODS[method]['type'] == 'rank':
            consensus_dfs[method] = CONSENSUS_METHODS[method]['function'](
                filtered_ranked_df, clustering_method, list(combination))
        elif CONSENSUS_METHODS[method]['type'] == 'score':
            consensus_dfs[method] = CONSENSUS_METHODS[method]['function'](
                filtered_standardised_df, clustering_method, list(combination))
        merged_df = pd.merge(consensus_dfs[method], actives_df, on='ID')
        # Get the column name that is not 'ID' or 'Activity'
        col_to_sort = [
            col for col in merged_df.columns if col not in ['ID', 'Activity']
        ][0]
        ...
        bedroc = round(
            Scoring.CalcBEDROC(list(zip(scores, activities)), 1, 80.5), 3)
...
def calculate_performance_for_clustering_method(dir, w_dir, actives_df,
                                                percentages):
    clustering_method = '_'.join(dir.split('_')[1:3]) if len(dir.split(
        '_')) > 3 else dir.split('_')[1] if len(dir.split('_')) == 3 else None
    ...
    score_columns = [
        col for col in standardised_df.columns if col not in ['Pose ID', 'ID']
    ]
    result_list = []
...
def calculate_performance(w_dir: Path, actives_library: Path,
                          percentages: list):
    """
    ...
    #Load actives data
    actives_df = PandasTools.LoadSDF(str(actives_library),
                                     molColName=None,
                                     idName='ID')
    ...
    dirs = [
        dir for dir in os.listdir(w_dir)
        if dir.startswith('rescoring') and dir.endswith('clustered')
    ]
