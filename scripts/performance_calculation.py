...
def process_dataframes(w_dir, rescoring_folders):
    rescored_dataframes = {name: pd.read_csv(Path(w_dir) / 'temp' / rescoring_folders[name] / 'allposes_rescored.csv') for name in rescoring_folders}
...
def apply_consensus_methods_combinations(w_dir, docking_library, clustering_metrics):
    ...
    for name, df_dict in {'standardised': standardised_dataframes, 'ranked': ranked_dataframes}.items():
        for df_name, df in df_dict.items():
            ...
            df.to_csv(Path(w_dir) / 'temp' / 'ranking' / f'{df_name}.csv', index=False)
    ...
    consensus_summary = pd.concat(df_list, ignore_index=True)
    consensus_summary.to_csv(Path(w_dir) / 'temp' / 'consensus' / 'consensus_summary.csv', index=False)
...
def calculate_EF_single_functions(w_dir, docking_library, clustering_metrics):
    ...
    (Path(w_dir) / 'temp' / 'consensus').mkdir(parents=True, exist_ok=True)
    EF_results.to_csv(Path(w_dir) / 'temp' / 'consensus' / 'EF_single_functions.csv', index=False)
...
def apply_consensus_methods(w_dir, clustering_metric, method, rescoring_functions):
    ...
    rescored_dataframe = pd.read_csv(Path(w_dir) / 'temp' / rescoring_folder / 'allposes_rescored.csv')
    ...
    analysed_dataframe.to_csv(Path(w_dir) / 'temp' / 'consensus' / f'{clustering_metric}_{method}_results.csv', index=False)
