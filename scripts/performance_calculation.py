...
def process_combination(combination, docking_program, selection_method, ranked_df, standardised_df, actives_df, percentages):
...
def calculate_performance_for_clustering_method_OLD(dir, w_dir, actives_df, percentages):
...
def calculate_performance(w_dir: Path, actives_library: Path, percentages: list):
	...
	results = parallel_executor(calculate_performance_for_clustering_method_OLD,
								dirs,
								n_cpus=1,
								job_manager="concurrent_process_silent",
								w_dir=w_dir,
...
def determine_optimal_conditions(w_dir: Path, results: dict, actives_library: Path, percentages: list):
	"""
	Determines the optimal conditions by calculating the performance of scoring systems
	for different docking programs and selection methods.

	Args:
		results (dict): A nested dictionary containing rescored poses for each docking program and selection method.
		actives_library (Path): The path to the actives library in SDF format.
		percentages (list): A list of percentages for calculating the EF (enrichment factor).

	Returns:
		DataFrame: A DataFrame containing the performance results for different docking programs,
				   selection methods, consensus methods, and scoring functions.
	"""
	...
	for docking_program, selection_methods in results.items():
		for selection_method, rescored_poses in selection_methods.items():
			printlog(f"Processing {docking_program} with {selection_method} selection method...")
			rescored_dataframe = pd.read_csv(rescored_poses)
			performance_results = calculate_performance_for_method(
				rescored_dataframe,
				actives_df,
				percentages,
				docking_program,
	...
	dm = skc.mkdm(
		matrix,
		objectives,
		alternatives=combined_results.index.tolist(),
		criteria=metrics,
	)
