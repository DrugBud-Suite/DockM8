...
def qvinaw_docking(protein_file, pocket_definition, software, exhaustiveness, n_poses):
    ...
    try:
        ...
        for sdf in sdf_files:
            df = PandasTools.LoadSDF(str(sdf), idName='ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=False)
            list_ = [*range(1, int(n_poses)+1, 1)]
            ser = list_ * (len(df) // len(list_))
            df['Pose ID'] = [f'{sdf.name.replace(".sdf", "")}_QVINAW_{num}' for num, (_, row) in zip(ser + list_[:len(df)-len(ser)], df.iterrows())]
        ...
        PandasTools.WriteSDF(qvinaw_poses, str(qvinaw_docking_results), molColName='Molecule', idName='Pose ID', properties=list(qvinaw_poses.columns))
    except Exception as e:
...
def qvina2_docking(protein_file, pocket_definition, software, exhaustiveness, n_poses):
    ...
    try:
        ...
        PandasTools.WriteSDF(qvina2_poses, str(qvina2_docking_results), molColName='Molecule', idName='Pose ID', properties=list(qvina2_poses.columns))
    except Exception as e:
...
def smina_docking(protein_file, pocket_definition, software, exhaustiveness, n_poses):
    ...
    try:
        smina_df = PandasTools.LoadSDF(str(results_path), idName='ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
        list_ = [*range(1, int(n_poses)+1, 1)]
        ...
        printlog(e)
    try:
        PandasTools.WriteSDF(smina_df, str(smina_folder / 'smina_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(smina_df.columns))
        toc = time.perf_counter()
...
def gnina_docking(protein_file, pocket_definition, software, exhaustiveness, n_poses):
    ...
    try:
        gnina_df = PandasTools.LoadSDF(str(results_path), idName='ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
        list_ = [*range(1, int(n_poses)+1, 1)]
        ...
        printlog(e)
    try:
        PandasTools.WriteSDF(gnina_df, str(gnina_folder / 'gnina_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(gnina_df.columns))
        toc = time.perf_counter()
...
def plants_docking(protein_file, pocket_definition, software, n_poses):
    ...
    try:
        obabel_command = 'obabel -isdf '+str(library)+' -O '+str(plants_library_mol2)
    ...
    try:
        ...
        plants_docking_command = 'cd '+str(software)+' && ./PLANTS --mode screen '+str(plants_docking_config_path)
    ...
    try:
        plants_poses = PandasTools.LoadSDF(str(plants_docking_results_sdf), idName='ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
        ...
        printlog(e)
    try:
        PandasTools.WriteSDF(plants_df, str(plants_folder / 'plants_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(plants_df.columns))
...
def docking(w_dir, protein_file, pocket_definition, software, docking_programs, exhaustiveness, n_poses, ncpus):
    if ncpus == 1:
        ...
        #Load all poses
        if (w_dir / 'temp' / 'allposes.sdf').is_file() == False:
            try:
                all_poses_list = []
                if 'GNINA' in docking_programs and (w_dir / 'temp' / 'gnina').is_dir() == True:
                    gnina_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'gnina' / 'gnina_poses.sdf'), idName='Pose ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    all_poses_list.append(gnina_df)
                if 'SMINA' in docking_programs and (w_dir / 'temp' / 'smina').is_dir() == True:
                    smina_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'smina' / 'smina_poses.sdf'), idName='Pose ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    all_poses_list.append(smina_df)
                if 'PLANTS' in docking_programs and (w_dir / 'temp' / 'plants').is_dir() == True:
                    plants_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'plants' / 'plants_poses.sdf'), idName='Pose ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    all_poses_list.append(plants_df)
                if 'QVINAW' in docking_programs and (w_dir / 'temp' / 'qvinaw').is_dir() == True:
                    qvinaw_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'qvinaw' / 'qvinaw_poses.sdf'), idName='Pose ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    all_poses_list.append(qvinaw_df)
                if 'QVINA2' in docking_programs and (w_dir / 'temp' / 'qvina2').is_dir() == True:
                    qvina2_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'qvina2' / 'qvina2_poses.sdf'), idName='Pose ID', molColName='Molecule',includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    all_poses_list.append(qvina2_df)
                all_poses = pd.concat(all_poses_list)
                PandasTools.WriteSDF(all_poses, str(w_dir / 'temp'/ 'allposes.sdf'), molColName='Molecule', idName='Pose ID', properties=list(all_poses.columns))
            except Exception as e:
            ...
            if 'PLANTS' in docking_programs and (w_dir / 'temp' / 'plants').is_dir() and not (w_dir / 'temp' / 'plants' / 'plants_poses.sdf').is_file():
                    ...
                    if item.startswith('results'):
                        ...
                        if file_path.is_file():
                            try:
                                ...
                                subprocess.call(obabel_command, shell=True, stdout=DEVNULL, stderr=STDOUT)
                                plants_poses = PandasTools.LoadSDF(str(file_path.with_suffix('.sdf')), idName='ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                ...
                try:
                    ...
                    PandasTools.WriteSDF(plants_df, str(w_dir / 'temp' / 'plants' / 'plants_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(plants_df.columns))
                    files = software.glob('*.pid')
                    for file in files:
                        file.unlink()
                except Exception as e:
                ...
                try:
                    ...
                    for file in tqdm(os.listdir(w_dir / 'temp' / 'smina'), desc='Loading SMINA poses'):
                        if file.startswith('split'):
                            df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'smina' / file), idName='ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    ...
                    printlog(e)
                try:
                    PandasTools.WriteSDF(smina_df, str(w_dir / 'temp' / 'smina' / 'smina_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(smina_df.columns))
                except Exception as e:
            ...
            if 'GNINA' in docking_programs and (w_dir / 'temp' / 'gnina').is_dir() and not (w_dir / 'temp' / 'gnina' / 'gnina_poses.sdf').is_file():
                try:
                    ...
                    for file in tqdm(os.listdir(w_dir / 'temp' / 'gnina'), desc='Loading GNINA poses'):
                        if file.startswith('split'):
                            df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'gnina' / file), idName='ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    ...
                    printlog(e)
                try:
                    PandasTools.WriteSDF(gnina_df, str(w_dir / 'temp' / 'gnina' / 'gnina_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(gnina_df.columns))
                except Exception as e:
            ...
            if 'QVINAW' in docking_programs and (w_dir / 'temp' / 'qvinaw').is_dir() and not (w_dir / 'temp' / 'qvinaw' / 'qvinaw_poses.sdf').is_file():
                try:
                    ...
                    for file in tqdm(os.listdir(w_dir / 'temp' / 'qvinaw'), desc='Loading QVINAW poses'):
                        if file.startswith('split'):
                            df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'qvinaw' / file), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    ...
                    printlog(e)
                try:
                    PandasTools.WriteSDF(qvinaw_df, str(w_dir / 'temp' / 'qvinaw' / 'qvinaw_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(qvinaw_df.columns))
                except Exception as e:
            ...
            if 'QVINA2' in docking_programs and (w_dir / 'temp' / 'qvina2').is_dir() and not (w_dir / 'temp' / 'qvina2' / 'qvina2_poses.sdf').is_file():
                try:
                    ...
                    for file in tqdm(os.listdir(w_dir / 'temp' / 'qvina2'), desc='Loading QVINA2 poses'):
                        if file.startswith('split'):
                            df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'qvina2' / file), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
                    ...
                    printlog(e)
                try:
                    PandasTools.WriteSDF(qvina2_df, str(w_dir / 'temp' / 'qvina2' / 'qvina2_poses.sdf'), molColName='Molecule', idName='Pose ID', properties=list(qvina2_df.columns))
                except Exception as e:
            ...
            if 'GNINA' in docking_programs and (w_dir / 'temp' / 'gnina' / 'gnina_poses.sdf').is_file():
                try:
                    gnina_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'gnina' / 'gnina_poses.sdf'), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
            ...
            if 'SMINA' in docking_programs and (w_dir / 'temp' / 'smina' / 'smina_poses.sdf').is_file():
                try:
                    smina_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'smina' / 'smina_poses.sdf'), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
            ...
            if 'PLANTS' in docking_programs and (w_dir / 'temp' / 'plants' / 'plants_poses.sdf').is_file():
                try:
                    plants_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'plants' / 'plants_poses.sdf'), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
            ...
            if 'QVINAW' in docking_programs and (w_dir / 'temp' / 'qvinaw' / 'qvinaw_poses.sdf').is_file():
                try:
                    qvinaw_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'qvinaw' / 'qvinaw_poses.sdf'), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
            ...
            if 'QVINA2' in docking_programs and (w_dir / 'temp' / 'qvina2' / 'qvina2_poses.sdf').is_file():
                try:
                    qvina2_df = PandasTools.LoadSDF(str(w_dir / 'temp' / 'qvina2' / 'qvina2_poses.sdf'), idName='Pose ID', molColName='Molecule', includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True)
