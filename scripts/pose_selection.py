...
def select_poses(
    ...
    try:
        ...
        elif selection_method.startswith("bestpose_"):
            program = selection_method.split("_")[1]
            if not any(all_poses["Docking_program"] == program):
                raise ValueError(f"Error during pose selection: No poses found from docking program: {program}")
            
            min_pose_indices = all_poses.groupby(["ID", "Docking_program"])["Pose_Number"].idxmin()
            selected_poses = all_poses.loc[min_pose_indices]
            selected_poses = selected_poses[selected_poses["Docking_program"] == program]
