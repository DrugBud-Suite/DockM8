...
def split_sdf(dir, sdf_file, ncpus):
    ...
    df = PandasTools.LoadSDF(
        str(sdf_file), molColName="Molecule", idName="ID", includeFingerprints=False, strictParsing=True
    )
...
def parallel_SDF_loader(
    sdf_path: Path, molcolName: str, idName: str, ncpus=os.cpu_count() - 2, SMILES=None
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        mols = [
            m
            for m in Chem.MultithreadedSDMolSupplier(
                sdf_path, numWriterThreads=ncpus, removeHs=False, strictParsing=True
            )
            if m is not None
        ]
        data = []
