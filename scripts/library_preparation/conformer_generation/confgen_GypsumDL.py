...
def generate_conformers_GypsumDL(input_sdf: Path, output_dir: Path, software: Path, n_cpus: int, n_conformers: int = 1):
	"""
		...
		for mol in input_mols:
			if mol.HasProp("_Name") and mol.GetProp("_Name") in missing_ids:
				try:
					...
					final_df = final_df.append({"Molecule": mol, "ID": mol.GetProp("_Name")}, ignore_index=True)
				except Exception as e:
					printlog(
