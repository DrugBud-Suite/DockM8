"""Implementation of the GNINA docking function."""
...
class GninaDocking(DockingFunction):
    """A class to perform docking using the GNINA software."""

    def __init__(self, software_path: Path):
        """Initializes the GninaDocking class.
    ...
    """Implementation of the GNINA docking function with statistics tracking fix."""
    ...
    def dock_batch(
        ...
        """Performs docking for a single batch of ligands using GNINA.

        Executes the GNINA docking command, processes the raw output,
        and saves the processed results to an SDF file.
        """
        ...
        try:
            ...
            if result.returncode != 0:
                printlog(f"GNINA docking failed for batch {batch_file.stem}")
                printlog(f"Error output: {result.stderr}")
                return None

            if not raw_results_path.exists() or raw_results_path.stat().st_size == 0:
                printlog(
                    f"GNINA output file is missing or empty for batch {batch_file.stem}"
                )
                return None
            ...
            try:
                ...
                printlog(f"Error loading raw SDF for {batch_file.stem}: {e}")
                return None
            ...
            # Clean up raw results after successful processing
            raw_results_path.unlink()
            return processed_results_path

        except Exception as e:
            printlog(
                f"ERROR: Failed to process GNINA result for batch {batch_file.stem}: {str(e)}"
            )
            return None
