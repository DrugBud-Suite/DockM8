...
def psovina_docking(
    ...
    for pdbqt_file in pdbqt_folder.glob("*.pdbqt"):
        ...
        psovina_cmd = (f"{software / 'psovina'}"
                       ...
                       " --cpu 1 --seed 1"
                       f" --num_modes {n_poses}")
        subprocess.call(psovina_cmd,
                        shell=True,
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT)

    psovina_docking_results = psovina_folder / (Path(input_file).stem +
                                                "_psovina.sdf")
    # Process the docked poses
    try:
        for file in results_folder.glob("*.pdbqt"):
            split_pdbqt_str(file)
        psovina_poses = pd.DataFrame(
            columns=["Pose ID", "Molecule", "PSOVINA_Affinity"])
        ...
        shutil.rmtree(psovina_folder / Path(input_file).stem,
                      ignore_errors=True)
...
def fetch_psovina_poses(w_dir: Union[str, Path], *args):
    ...
    """
    if (w_dir / "psovina").is_dir() and not (w_dir / "psovina" /
                                             "psovina_poses.sdf").is_file():
        try:
            psovina_dataframes = []
