...
def vinaXB_docking(
    ...
    """
    Perform docking using the VINAXB software for either a library of molecules or split files.
    ...
    Args:
        ...
        software (Path): Path to the VINAXB software folder.
    ...
    for pdbqt_file in pdbqt_folder.glob("*.pdbqt"):
        ...
        subprocess.call(
            vinaXB_cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
        )

    vinaXB_docking_results = vinaXB_folder / (Path(input_file).stem + "_vinaXB.sdf")
    # Process the docked poses
    try:
        for file in results_folder.glob("*.pdbqt"):
            split_pdbqt_str(file)
        vinaXB_poses = pd.DataFrame(columns=["Pose ID", "Molecule", "VINAXB_Affinity"])
        for pose_file in results_folder.glob("*.pdbqt"):
            ...
            # Extracting VINAXB_Affinity from the file
            ...
            vinaXB_poses.loc[vinaXB_poses.shape[0]] = {
                "Pose ID": pose_file.stem,
                "Molecule": rdkit_mol[0],
                "VINAXB_Affinity": affinity,
                "ID": pose_file.stem.split("_")[0],
            }
        PandasTools.WriteSDF(
            vinaXB_poses,
            str(vinaXB_docking_results),
            molColName="Molecule",
            idName="Pose ID",
            properties=list(vinaXB_poses.columns),
        )
    except Exception as e:
        printlog("ERROR: Failed to combine VINAXB SDF file!")
        printlog(e)
    else:
        shutil.rmtree(vinaXB_folder / Path(input_file).stem, ignore_errors=True)
    return vinaXB_docking_results


def fetch_vinaXB_poses(w_dir: Union[str, Path], *args):
    """
    Fetches VINAXB poses from the specified directory and combines them into a single SDF file.

    Args:
        w_dir (str or Path): The directory path where the VINAXB poses are located.

    Returns:
        Path: The path to the combined VINAXB poses SDF file.

    Raises:
        Exception: If there is an error loading or writing the VINAXB poses SDF file.
    """
    if (w_dir / "vinaXB").is_dir() and not (
        w_dir / "vinaXB" / "vinaXB_poses.sdf"
    ).is_file():
        try:
            vinaXB_dataframes = []
            for file in tqdm(os.listdir(w_dir / "vinaXB"), desc="Loading VINAXB poses"):
                if (
                    ...
                    df = PandasTools.LoadSDF(
                        str(w_dir / "vinaXB" / file),
                        idName="Pose ID",
                        molColName="Molecule",
                    )
                    vinaXB_dataframes.append(df)
            vinaXB_df = pd.concat(vinaXB_dataframes)
        except Exception as e:
            printlog("ERROR: Failed to Load VINAXB poses SDF file!")
            printlog(e)
        try:
            PandasTools.WriteSDF(
                vinaXB_df,
                str(w_dir / "vinaXB" / "vinaXB_poses.sdf"),
                molColName="Molecule",
                idName="Pose ID",
                properties=list(vinaXB_df.columns),
            )
        except Exception as e:
            printlog("ERROR: Failed to write combined VINAXB poses SDF file!")
            printlog(e)
        else:
            delete_files(w_dir / "vinaXB", "vinaXB_poses.sdf")
    return w_dir / "vinaXB" / "vinaXB_poses.sdf"
