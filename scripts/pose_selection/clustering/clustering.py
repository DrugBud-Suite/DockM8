...
def calculate_and_cluster(df: pd.DataFrame, selection_method: str, clustering_method: str, protein_file: str) -> pd.DataFrame:
    """
...
def run_clustering(all_poses: pd.DataFrame, selection_method: str, clustering_method: str, protein_file: Path, n_cpus: int) -> pd.DataFrame:
    """
    ...
    with pebble.ProcessPool(max_workers=n_cpus) as executor:
        ...
        for current_id in tqdm(id_list, desc=f"Submitting {selection_method} jobs...", unit="IDs"):
            try:
                ...
                job = executor.schedule(calculate_and_cluster, args=(all_poses[all_poses["ID"] == current_id], selection_method, clustering_method, protein_file,), timeout=120,)
                jobs.append(job)
