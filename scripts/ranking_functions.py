...
def standardise_scores(dataframe):
    columns_to_standardize = ['GNINA_Affinity','Vinardo_Affinity','AD4_Affinity','PLP','CHEMPLP']
    dataframe[columns_to_standardize] = np.abs(dataframe[columns_to_standardize] - dataframe[columns_to_standardize].max())
    return dataframe[sorted(dataframe.columns)]

def standardise_rank_scores(dataframe, clustering_method):
    columns_to_standardize = ['GNINA_Affinity','Vinardo_Affinity','AD4_Affinity','PLP','CHEMPLP']
    dataframe[columns_to_standardize] = np.abs(dataframe[columns_to_standardize] - dataframe[columns_to_standardize].max())
    dataframe = dataframe.assign(**{f'{col}_RANK_{clustering_method}': dataframe[col].rank(method='average', ascending=False) for col in dataframe.columns if col != 'Pose ID'})
...
def apply_ranking_methods_simplified(w_dir, clustering_metrics):
    ...
    standardised_dataframes = {name+'_standardised': standardise_scores(rescored_dataframes[name], name) for name in rescoring_folders}
    ranked_dataframes = {name+'_ranked': standardise_rank_scores(rescored_dataframes[name], name) for name in rescoring_folders}
