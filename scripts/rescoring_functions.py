...
def rescore_all(w_dir, protein_file, pocket_definition, software, clustered_sdf, functions, ncpus):
    ...
    rescoring_folder = Path(w_dir) / 'temp' / f'rescoring_{rescoring_folder_name}'
    ...
    def gnina_rescoring(sdf, ncpus):
        ...
        if ncpus == 1:
            ...
            results = rescoring_folder / 'gnina_rescoring' / f'rescored_{cnn}.sdf'
            gnina_cmd = (
                f'cd {software} && ./gnina'
            ...
            gnina_rescoring_results = PandasTools.LoadSDF(str(results), idName='Pose ID', molColName=None, includeFingerprints=False, removeHs=False)
        else:
            split_files_folder = split_sdf(rescoring_folder / 'gnina_rescoring', sdf, ncpus)
            split_files_sdfs = [split_files_folder / f for f in os.listdir(split_files_folder) if f.endswith('.sdf')]
            printlog('Rescoring with GNINA')
            global gnina_rescoring_splitted
            def gnina_rescoring_splitted(split_file, protein_file, pocket_definition, software):
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=ncpus) as executor:
                ...
                for split_file in tqdm(split_files_sdfs, desc='Submitting GNINA rescoring jobs', unit='file'):
                    try:
                        job = executor.submit(gnina_rescoring_splitted, split_file, protein_file, pocket_definition, software)
                        jobs.append(job)
                    except Exception as e:
    ...
    def vinardo_rescoring(sdf, ncpus):
        ...
        vinardo_rescoring_results.rename(columns={'minimizedAffinity': 'Vinardo'}, inplace=True)
        vinardo_rescoring_results = vinardo_rescoring_results[['Pose ID', 'Vinardo']]
    ...
    def AD4_rescoring(sdf, ncpus):
        ...
        AD4_rescoring_results.rename(columns = {'minimizedAffinity':'AD4'}, inplace = True)
    ...
    def rfscore_rescoring(sdf, ncpus):
        ...
        rfscorevs_rescoring_folder = Path(rescoring_folder) / 'rfscorevs_rescoring'
    ...
    def RTMScore_rescoring(sdf, ncpus):
        ...
        if ncpus == 1:
            ...
            try:
                results = rtmscore(prot=RTMScore_pocket, lig=sdf, output=RTMScore_rescoring_folder / 'RTMScore_scores.csv', model=str(software / 'RTMScore/trained_models/rtmscore_model1.pth'), ncpus=1)
            except:
                printlog('RTMScore scoring with pocket failed, scoring with whole protein...')
                results = rtmscore(prot=protein_file, lig=sdf, output=RTMScore_rescoring_folder / 'RTMScore_scores.csv', model=str(software / 'RTMScore/trained_models/rtmscore_model1.pth'), ncpus=1)
        else:
            ...
            def RTMScore_rescoring_splitted(split_file, protein_file, software, ncpus):
                output_file = RTMScore_rescoring_folder / f'{split_file.stem}_RTMScore.csv'
                try:
    ...
    def LinF9_rescoring(sdf, ncpus):
        ...
        if ncpus == 1:
            ...
            def LinF9_rescoring_splitted(split_file, protein_file, pocket_definition, software):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=ncpus) as executor:
            ...
            for split_file in tqdm(split_files_sdfs, desc='Submitting LinF9 rescoring jobs', unit='file'):
                try:
                    job = executor.submit(LinF9_rescoring_splitted, split_file, protein_file, pocket_definition, software)
                    jobs.append(job)
                except Exception as e:
                    printlog("Error in concurrent futures job creation: " + str(e))
        ...
        try:
            LinF9_dataframes = [PandasTools.LoadSDF(str(rescoring_folder / 'LinF9_rescoring' / file), idName='Pose ID', molColName=None, includeFingerprints=False, embedProps=False, removeHs=False, strictParsing=True) for file in os.listdir(rescoring_folder / 'LinF9_rescoring') if file.startswith('split') and file.endswith('.sdf')]
        except Exception as e:
    ...
    def AAScore_rescoring(sdf, ncpus):
        ...
        if ncpus == 1:
            ...
            split_files_folder = split_sdf(AAScore_rescoring_folder, sdf, ncpus)
            split_files_sdfs = [Path(split_files_folder) / f for f in os.listdir(split_files_folder) if f.endswith('.sdf')]
