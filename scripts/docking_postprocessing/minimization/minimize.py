...
def minimize_all_ligands(pdb_path: str, input_data: Union[str, pd.DataFrame],
							n_cpus: int = int(os.cpu_count() * 0.9)) -> pd.DataFrame:
	"""
    Minimize all ligands in an SDF file or DataFrame in the context of a protein pocket.

    Parameters:
    pdb_path (str): Path to the input protein PDB file.
    input_data (Union[str, pd.DataFrame]): Path to the input SDF file containing ligands or a pre-loaded DataFrame.
    n_cpus (int): Number of CPUs to use for parallel processing.

    Returns:
    pd.DataFrame: DataFrame containing minimized ligands.
    """
	# Check if input_data is a file path or a DataFrame
	if isinstance(input_data, str):
		# Read the ligands from the SDF file
		dataframe = PandasTools.LoadSDF(input_data, molColName='Molecule', idName='Pose ID')
