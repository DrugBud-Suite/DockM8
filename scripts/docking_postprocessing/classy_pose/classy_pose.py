...
def classy_pose(dataframe: pd.DataFrame, protein_file: Path, model_file: Path, n_cpus: int) -> pd.DataFrame:
	"""
	...
	mols = [oddt.toolkit.rdk.Molecule(mol) for mol in dataframe['Molecule']]
	...
	full_dataframe = pd.concat([dataframe, plec_dataframe], axis=1)

	test_data = full_dataframe.drop(columns=['Pose ID', 'Molecule'], axis=1)
	...
	full_dataframe['Good_Pose_Prob'] = prediction_test_prob

	predicted_class = ['Good' if prob > 0.5 else 'Bad' for prob in prediction_test_prob]

	full_dataframe['Predicted_Class'] = predicted_class

	filtered_dataframe = full_dataframe[full_dataframe['Predicted_Class'] == 'Good']
