...
def calculate_and_cluster(
    clustering_metric: str, clustering_method: str, df: pd.DataFrame, protein_file: str
) -> pd.DataFrame:
    """
    ...
    if not cluster_file.exists():
        ...
        if selection_method == "bestpose":
            ...
            with pebble.ProcessPool(max_workers=ncpus) as executor:
                ...
                for job in tqdm(
                    jobs, total=len(id_list), desc=f"Running {selection_method} clustering...", unit="jobs"
                ):
                    try:
