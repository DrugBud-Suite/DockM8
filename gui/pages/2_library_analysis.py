...
if st.button('Load Library'):
	with st.spinner('Loading library...'):
		if Path(input_library).is_file():
			ligand_library = PandasTools.LoadSDF(input_library, smilesName='SMILES', molColName='Molecule', idName='ID')
...
if 'ligand_library' in st.session_state:
	...
	if st.button('Calculate Properties'):
		with st.spinner('Calculating properties...'):
			try:
				st.session_state['calculated_ligand_library'] = calculate_properties(
					st.session_state['ligand_library'], properties)
	...
	if 'calculated_ligand_library' in st.session_state:
		...
		if st.button('Save Calculated Properties to SDF', key='save_calculated_properties_button'):
			with st.spinner('Saving properties to SDF...'):
...
if 'ligand_library' in st.session_state:
	...
	col1, col2 = st.columns(2)
	if 'filtered_ligand_library' in st.session_state:
		if st.button('Save Filtered Library to SDF', key='save_final_filtered_library_button'):
			with st.spinner('Saving filtered library to SDF...'):
