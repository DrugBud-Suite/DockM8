...
class Ligand:
    def __init__(self, docking_library, id_column, software_path, protonation_option, number_cpus):
        self.docking_library = docking_library
        self.id_column = id_column
        self.software_path = software_path
        self.protonation_option = protonation_option
        self.number_cpus = number_cpus
        self.standardized_sdf = os.path.dirname(self.docking_library)+'/temp/standardized_library.sdf'
        self.protonated_sdf = os.path.dirname(self.docking_library)+'/temp/protonated_library.sdf'
        self.standardized_df = None
    
    def standardization(self):
        if os.path.isfile(self.standardized_sdf) == False:
            self.standardize_library_futures(self.docking_library, self.id_column, self.number_cpus)
    ...
    def protonation(self):
        if os.path.isfile(self.protonated_sdf) == False:
            if self.protonation_option == 'pkasolver':
                ...
                self.generate_conformers_GypsumDL_noprotonation(self.protonated_sdf)
            elif self.protonation_option == 'GypsumDL':
                self.generate_conformers_GypsumDL_withprotonation(self.standardized_sdf)
            else:
                self.generate_conformers_GypsumDL_noprotonation(self.standardized_sdf)
        else:
            self.generate_conformers_GypsumDL_noprotonation(self.protonated_sdf)
        self.cleanup()
    ...
    def standardize_library_futures(self):
        ...
        try:
            df = PandasTools.LoadSDF(self.docking_library, molColName='Molecule', idName=self.id_column, removeHs=True, strictParsing=True, smilesName='SMILES') #gotta rename id column to ID
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=self.number_cpus) as executor:
            df['Molecule'] = list(tqdm.tqdm(executor.map(self.standardize_molecule, df['Molecule']), total=len(df['Molecule']), desc='Standardizing molecules', unit='mol'))
        ...
        PandasTools.WriteSDF(df, self.standardized_sdf, molColName='Molecule', idName='ID')
    ...
    def protonate_library_pkasolver(self):
        ...
        try:
            if self.standardized_df is None:
                input_df = PandasTools.LoadSDF(self.standardized_sdf, molColName=None, idName='ID', removeHs=True, strictParsing=True, smilesName='SMILES')
        ...
        print(f'Standardization of compound library finished: Started with {n_cpds_start}, ended with {n_cpds_end} : {n_cpds_start-n_cpds_end} compounds lost')
        PandasTools.WriteSDF(protonated_df, self.protonated_sdf, molColName='Molecule', idName='ID')
    ...
    def generate_conformers_GypsumDL_withprotonation(self,sdf_path):
        ...
        try:
            gypsum_dl_command = 'python '+self.software_path+'/gypsum_dl-1.2.0/run_gypsum_dl.py -s '+sdf_path+' -o '+os.path.dirname(sdf_path)+' --job_manager multiprocessing -p '+str(self.number_cpus)+' -m 1 -t 10 --min_ph 6.5 --max_ph 7.5 --pka_precision 1 --skip_alternate_ring_conformations --skip_making_tautomers --skip_enumerate_chiral_mol --skip_enumerate_double_bonds --max_variants_per_compound 1'
    ...
    def generate_conformers_GypsumDL_noprotonation(self,sdf_path):
        ...
        try:
            gypsum_dl_command = 'python '+software_path+'/gypsum_dl-1.2.0/run_gypsum_dl.py -s '+sdf_path+' -o '+os.path.dirname(sdf_path)+' --job_manager multiprocessing -p '+str(self.number_cpus)+' -m 1 -t 10 --skip_adding_hydrogen --skip_alternate_ring_conformations --skip_making_tautomers --skip_enumerate_chiral_mol --skip_enumerate_double_bonds --max_variants_per_compound 1'
            subprocess.call(gypsum_dl_command, shell=True, stdout=DEVNULL, stderr=STDOUT)
        except Exception as e: 
