...
class Protein:
    ...
    def load_molecule(self):
        ...
        if self.ligand_file.endswith('.mol2'):
            self.ligand_mol = Chem.MolFromMol2File(self.ligand_file, sanitize=False, removeHs=False)
        if self.ligand_file.endswith('.mol'):
            self.ligand_mol = Chem.MolFromMolFile(self.ligand_file, sanitize=False, removeHs=False)
        elif self.ligand_file.endswith('.sdf'):
            supplier = Chem.SDMolSupplier(self.ligand_file, sanitize=False, removeHs=False)
            ...
            for line in pdbqt_data:
                pdb_block += '{}\n'.format(line[:66])
            self.ligand_mol = Chem.MolFromPDBBlock(pdb_block, sanitize=False, removeHs=False)
        elif self.ligand_file.endswith('.pdb'):
            self.ligand_mol= Chem.MolFromPDBFile(self.ligand_file, sanitize=False, removeHs=False)
        else:
    ...
    def process_pro_and_lig(self):
        ...
        protein_cut = self.select_cut_residue(protein_biop, self.ligand_mol, self.cut)
    ...
    def select_cut_residue(self, protein_biop, ligand_mol, cut):
        ...
        protein_df['chain_rid'] = protein_df.apply(lambda row: 
    ...
    def cal_pro_min_dist(self, protein_biop, ligand_mol):
        ...
        protein_biop['min_dist']= protein_biop.apply(lambda row: self.get_min_dist(row['xyz'], ligu), axis=1)
        return protein_biop
    ...
    def add_xyz(self,ligu):
        ligu['xyz'] = ligu.apply(lambda row: self.conc(row['x_coord'],row['y_coord'],row['z_coord']),axis=1)
        return ligu
    ...
    def get_min_dist(self,am, ligu):
        ...
        ligu['dist']= ligu.apply(lambda row: self.cal_dist(row['xyz'], row['pro_xyz']), axis =1)
        md = min(ligu['dist'])
        return md
