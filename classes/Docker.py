...
class Docker:
    def __init__(
        ...
        self.docking_programs = {
            'GNINA': self.working_directory + '/temp/gnina/',
            'SMINA': self.working_directory + '/temp/smina/',
            'PLANTS': self.working_directory + '/temp/plants/'}
    ...
    def dock(self, program):
        ...
        if not self.parallelization:
            ...
            if not os.path.isdir(
                    ...
                    '/temp/split_final_library'):
                split_files_folder = self.split_sdf(
                    self.working_directory +
                    '/temp',
                    self.working_directory +
            ...
            if program == 'PLANTS':
                binding_site_x, binding_site_y, binding_site_z, binding_site_radius = self.plants_preparation(
                    self.protein_file, self.ligand_file, self.software)
                # Convert prepared ligand file to .mol2 using open babel
                for file in split_files_sdfs:
                    try:
                        obabel_command = 'obabel -isdf ' + split_files_folder + '/' + file + ' -O ' + \
                            w_dir + '/temp/plants/' + os.path.basename(file).replace('.sdf', '.mol2')
                        subprocess.call(
                            obabel_command,
                            shell=True,
            ...
            if program == 'SMINA':
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=self.number_cpus) as executor:
                    ...
                    for split_file in tqdm(
                        ...
                        try:
                            job = executor.submit(
                                self.smina_docking_splitted, split_file)
                            jobs.append(job)
                        except Exception as e:
                            print(
                                "Error in concurrent futures job creation: ", str(e))
            ...
            if program == 'GNINA':
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=self.number_cpus) as executor:
                    ...
                    for split_file in tqdm(
                        ...
                        try:
                            job = executor.submit(
                                self.gnina_docking_splitted, split_file)
                            jobs.append(job)
                        except Exception as e:
                            print(
                                "Error in concurrent futures job creation: ", str(e))
    ...
    def smina_docking(self):
        ...
        smina_cmd = 'cd ' + self.software + ' && ./gnina -r ' + self.protein_file + ' -l ' + self.library + ' --autobox_ligand ' + self.ligand_file + ' -o ' + \
            results_path + ' --exhaustiveness ' + str(self.exhaustiveness) + ' --num_modes ' + str(self.number_of_poses) + ' --cnn_scoring none --no_gpu --log ' + log
        try:
            subprocess.call(
                smina_cmd,
                shell=True,
    ...
    def gnina_docking(self):
        ...
        gnina_cmd = 'cd ' + self.software + ' && ./gnina -r ' + self.protein_file + ' -l ' + self.library + ' --autobox_ligand ' + self.ligand_file + ' -o ' + results_path + \
            ' --exhaustiveness ' + str(self.exhaustiveness) + ' --num_modes ' + str(self.number_of_poses) + ' --cnn_scoring rescore --cnn crossdock_default2018 --no_gpu --log ' + log
        try:
            subprocess.call(
                gnina_cmd,
                shell=True,
    ...
    def plants_docking(self):
        ...
        if os.path.isdir(plants_docking_dir):
        ...
        try:
            if self.ligand_file.endswith('.mol2'):
                ...
                print(
                    f'Converting reference file from .{self.ligand_file.split(".")[-1]} to .mol2 format for PLANTS docking...')
        ...
        try:
            ...
            plants_binding_site_command = 'cd ' + self.software + \
                ' && ./PLANTS --mode bind ' + plants_ref_mol2 + ' 8'
        ...
        plants_docking_config_path_config = plants_docking_config_path_txt.replace(
            ".txt", ".config")
        ...
        try:
            ...
            plants_docking_command = "cd " + self.software + \
                " && ./PLANTS --mode screen " + plants_docking_config_path_config
            subprocess.call(
            ...
            print(e)
        plants_docking_results_mol2 = self.working_directory + \
            "/temp/plants/results/docked_ligands.mol2"
        plants_docking_results_sdf = plants_docking_results_mol2.replace(
            ".mol2", ".sdf")
        # Convert PLANTS poses to sdf
        try:
            print('Converting PLANTS poses to .sdf format...')
            obabel_command = 'obabel -imol2 ' + plants_docking_results_mol2 + \
                ' -O ' + plants_docking_results_sdf
            subprocess.call(
                obabel_command,
                shell=True,
                stdout=subprocess.DEVNULL,
    ...
    def fetch_poses(self):
        ...
        if not self.parallelization:
            ...
            plants_poses_results_sdf = self.working_directory + \
                "/temp/plants/results/docked_ligands.sdf"
            plants_scoring_results_sdf = self.working_directory + \
                "/temp/plants/results/ranking.csv"
            all_poses = pd.DataFrame()
            # Fetch PLANTS poses
            print('Fetching PLANTS poses...')
            try:
                plants_poses = PandasTools.LoadSDF(
                    plants_poses_results_sdf,
                    idName='ID',
                    molColName='Molecule',
                    includeFingerprints=False,
                    embedProps=False,
                ...
                plants_scores = pd.read_csv(
                    plants_scoring_results_sdf, usecols=[
    ...
    def smina_docking_splitted(self, split_file, software):
        ...
        results_path = smina_folder + \
            os.path.basename(split_file).split('.')[0] + '_smina.sdf'
        smina_cmd = 'cd ' + software + ' && ./gnina -r ' + self.protein_file + ' -l ' + split_file + ' --autobox_ligand ' + self.ligand_file + ' -o ' + \
            results_path + ' --exhaustiveness ' + str(self.exhaustiveness) + ' --num_modes ' + str(self.number_of_poses) + ' --cnn_scoring none --cpu 1 --no_gpu'
        try:
            subprocess.call(
                smina_cmd,
                shell=True,
    ...
    def gnina_docking_splitted(self, split_file, software):
        ...
        results_path = gnina_folder + \
            os.path.basename(split_file).split('.')[0] + '_gnina.sdf'
        gnina_cmd = 'cd ' + software + ' && ./gnina -r ' + self.protein_file + ' -l ' + split_file + ' --autobox_ligand ' + self.ligand_file + ' -o ' + results_path + \
            ' --exhaustiveness ' + str(self.exhaustiveness) + ' --num_modes ' + str(self.number_of_poses) + ' --cnn_scoring rescore --cnn crossdock_default2018 --cpu 1 --no_gpu'
        try:
            subprocess.call(
                gnina_cmd,
                shell=True,
    ...
    def plants_docking_splitted(
            ...
            binding_site_z,
            binding_site_radius):
        plants_docking_results_dir = self.working_directory + \
            '/temp/plants/results_' + os.path.basename(split_file).replace('.sdf', '')
        # Generate plants config file
        plants_docking_config_path_txt = self.working_directory + \
            '/temp/plants/config_' + os.path.basename(split_file).replace('.sdf', '.txt')
        plants_config = [
            '# search algorithm\n',
            'search_speed 1\n',
            'aco_ants 20\n',
            ...
            'protein_file ' +
            self.working_directory +
            '/temp/plants/protein.mol2' +
            '\n',
            'ligand_file ' +
            self.working_directory +
            '/temp/plants/' +
            os.path.basename(split_file).replace(
                '.sdf',
                '.mol2') +
            '\n',
            '# output\n',
        ...
        # Run PLANTS docking
        try:
            plants_docking_command = 'cd ' + software + \
                ' && ./PLANTS --mode screen ' + plants_docking_config_path_config
            subprocess.call(
                plants_docking_command,
                shell=True,
    ...
    def plants_preparation(self, software):
        ...
        try:
            print('Determining binding site coordinates using PLANTS...')
            plants_binding_site_command = 'cd ' + software + \
                ' && ./PLANTS --mode bind ' + plants_ref_mol2 + ' 8'
            run_plants_binding_site = os.popen(plants_binding_site_command)
            output_plants_binding_site = run_plants_binding_site.readlines()
            keep = []
        ...
        for item in tqdm(
            ...
            if item.startswith('results'):
                file_path = os.path.join(
                    plants_dir, item, 'docked_ligands.mol2')
                if os.path.isfile(file_path):
                    try:
                        ...
                        plants_poses = PandasTools.LoadSDF(
                            ...
                            removeHs=False,
                            strictParsing=True)
                        plants_scores = pd.read_csv(file_path.replace('docked_ligands.mol2', 'ranking.csv')).rename(
