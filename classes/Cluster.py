...
class Cluster:
    ...
    def cluster(self, metric):
        ...
        if os.path.isfile(
            ...
            clustered_poses = pd.merge(
                all_poses, clustered_poses, on='Pose ID')
    ...
    def matrix_calculation_and_clustering_futures_failure_handling(
            self, metric, method, df):
        ...
        if metric == '3DScore':
            ...
            vectorized_calc_vec = np.vectorize(
                metric_calculation_failure_handling)
            results = vectorized_calc_vec(
                subsets[:, 0], subsets[:, 1], 'spyRMSD', protein_file)
            ...
            output_df['Pose ID'] = output_df['Pose ID'].astype(
                str).str.replace('[()\',]', '', regex=False)
            return output_df
        else:
            subsets = np.array(list(itertools.combinations(df['Molecule'], 2)))
            ...
            vectorized_calc_vec = np.vectorize(
                metric_calculation_failure_handling)
            results = vectorized_calc_vec(
                subsets[:, 0], subsets[:, 1], metric, protein_file)
            i, j = np.array([indices[x] for x in subsets[:, 0]]), np.array(
                [indices[y] for y in subsets[:, 1]])
            matrix = np.zeros((len(df), len(df)))
            matrix[i, j] = results
        ...
        if metric in best_pose_filters:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=ncpus) as executor:
                ...
                for current_id in tqdm(
                        id_list,
                        desc='Submitting parallel jobs...',
                        unit='IDs'):
                    try:
                        job = executor.submit(matrix_calculation_and_clustering_futures_failure_handling,
                                              metric, method, all_poses[all_poses['ID'] == current_id], protein_file)
                        jobs.append(job)
                    except Exception as e:
                        print(
