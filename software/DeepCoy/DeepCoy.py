...
class DenseGGNNChemModel(ChemModel):
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions, edge_type_predictions):
        ...
        # Don't reweigh - vanilla cross entropy
            ...
            edge_loss =- tf.reduce_sum(tf.compat.v1.log(edge_softmax + SMALL_NUMBER) * edge_labels, axis=1)

            # edge type
            edge_type_loss =- edge_type_labels * tf.compat.v1.log(edge_type_probs + SMALL_NUMBER) # [b, e, v]
            edge_type_loss = tf.reduce_sum(edge_type_loss, axis=[1,2]) # [b]
