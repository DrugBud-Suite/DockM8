...
def generate_mask(valences, adj_mat, color, real_n_vertices, node_in_focus,
                  check_overlap_edge, new_mol):
    edge_type_mask = []
    edge_mask = []
...
def generate_label(ground_truth_graph, incremental_adj, node_in_focus,
                   real_neighbor, real_n_vertices, params):
    edge_type_label = []
    edge_label = []
    for neighbor in range(real_n_vertices):
        adjacent, edge_type = check_adjacent_sparse(ground_truth_graph,
                                                    node_in_focus, neighbor)
        incre_adjacent, incre_edge_type = check_adjacent_sparse(
            incremental_adj, node_in_focus, neighbor)
        if not params["label_one_hot"] and adjacent and not incre_adjacent:
            ...
            edge_label.append((node_in_focus, neighbor))
                ...
                "label_one_hot"] and adjacent and not incre_adjacent and neighbor == real_neighbor:
            edge_type_label.append((node_in_focus, neighbor, edge_type))
            edge_label.append((node_in_focus, neighbor))
...
def update_one_step(overlapped_edge_features, distance_to_others, node_sequence,
    ...
    edge_type_mask, edge_mask = generate_mask(valences, up_to_date_adj_mat,
                                              color, real_n_vertices,
                                              node_in_focus, check_overlap_edge,
                                              new_mol)
    edge_type_masks.append(edge_type_mask)
    edge_masks.append(edge_mask)
    if not local_stop_label:
        # generate the label based on ground truth graph
        edge_type_label, edge_label = generate_label(graph, up_to_date_adj_mat,
                                                     node_in_focus, neighbor,
...
def construct_incremental_graph(dataset,
    ...
    valences = get_initial_valence(
        [np.argmax(symbol) for symbol in node_symbol], dataset)
    # Add backward edges
    ...
    # Add atoms
    add_atoms(new_mol,
              sample_node_symbol([node_symbol], [len(node_symbol)], dataset)[0],
              dataset)
    # calculate keep probability
    sample_transition_count = real_n_vertices + len(edges) / 2
    keep_prob = float(sample_transition_count) / (
        (real_n_vertices + len(edges) / 2) * params["bfs_path_count"]
    )  # to form a binomial distribution
    while len(queue) > 0:
...
# there is no edge so far between node_in_focus and neighbor and it satisfy the valence constraint
# and node_in_focus != neighbor
def generate_frequencies(valences, adj_mat, color, real_n_vertices,
                         node_in_focus, check_overlap_edge, new_mol, freq_dict):
            ...
            for v in range(min_valence):
                ...
                while submol_smiles == "":
                    env = Chem.FindAtomEnvironmentOfRadiusN(
                        new_mol, radius, int(node_in_focus))
                    amap = {}
                ...
                if submol_smiles in freq_dict:
                    ...
                    transition_frequences_edge.append(
                        (neighbor, v, freq_dict[submol_smiles]))
                else:
                    score += 0
...
def update_one_step_freqs(overlapped_edge_features, distance_to_others,
    ...
    transition_frequences, transition_frequences_edge = generate_frequencies(
        valences, up_to_date_adj_mat, color, real_n_vertices, node_in_focus,
        check_overlap_edge, new_mol, freq_dict)
...
def construct_incremental_graph_freqs(dataset,
                                      edges,
                                      max_n_vertices,
                                      real_n_vertices,
                                      node_symbol,
