...
def calc_props_basic(smiles):
    try:
        ...
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#6]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num nitrogen
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#7]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num oxygen
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#8]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num fluorine
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#9]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num sulfur
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#16]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num chlorine
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#17]"),
                                        maxMatches=mol.GetNumAtoms())))
...
def calc_props_muv(smiles):
    try:
        ...
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#5]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num carbons
        props.append(
            len(
        ...
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#15]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num sulfur
        props.append(
            len(
        ...
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#35]"),
                                        maxMatches=mol.GetNumAtoms())))
        # num iodine
        props.append(
            len(
                mol.GetSubstructMatches(Chem.MolFromSmarts("[#53]"),
                                        maxMatches=mol.GetNumAtoms())))
...
def doe_score(actives, decoys):
    ...
    for i, act in enumerate(active_norm):
        comp = list(all_norm)
        del comp[i]
        dists = [100 - np.linalg.norm(c - act) for c in comp
                ]  # arbitrary large number to get scores in reverse order
...
def calc_xval_performance(active_props,
                          decoy_props,
                          random_state=42,
                          n_jobs=-1):
    ...
    # Scale
    in_props_scaled = [(active_feat - active_min_all) / active_scale_all
                       for active_feat in active_props]
    gen_props_scaled = [(gen_feat - active_min_all) / active_scale_all
                        for gen_feat in decoy_props]

    # Features
    X = np.squeeze(np.concatenate(
