...
def generate_decoys(input_sdf: Path, n_decoys: int, model: str,
    ...
    if not os.path.exists(DeepCoy_folder / "actives.smi"):
        try:
            # Convert to SMILES
            convert_molecules(input_sdf, DeepCoy_folder / "actives.smi", "sdf", "smi", software)
            ...
            printlog(f"Error converting library to SMILES for decoy generation: {str(e)}")
            return None
    ...
    if not os.path.exists(DeepCoy_folder / "molecules_actives.json"):
        try:
            ...
            printlog(f"Error preprocessing library for decoy generation: {str(e)}")
            return None
    ...
    if not os.path.exists(DeepCoy_folder / "decoys.smi"):
        try:
            ...
            printlog(f"Error generating decoys: {str(e)}")
            return None
    ...
    if not os.path.exists(DeepCoy_folder / "decoys-selected.smi"):
        try:
            ...
            printlog(f"Error selecting and evaluating decoys: {str(e)}")
            return None
    ...
    if not os.path.exists(DeepCoy_folder / "test_set.sdf"):
        try:
            ...
            printlog(f"Error creating test set SDF: {str(e)}")
            return None
