...
def graph_to_adj_mat(graph,
                     max_n_vertices,
                     num_edge_types,
                     tie_fwd_bkwd=True,
                     considering_edge_type=True):
    if considering_edge_type:
...
def sample_node_keep_new(node_keep_probs, real_length):
    ...
    node_keep_idxs = np.argsort(
        node_keep_probs[0][0:real_length])[-num_to_keep:]
...
def align_smiles_by_MCS(smiles_1, smiles_2):
    ...
    for mol in mols:
        sub_idx = list(
            mol.GetSubstructMatch(Chem.MolFromSmarts(res.smartsString)))
...
def shape_count(dataset, remove_print=False, all_smiles=None):
    ...
    geometry_counts_per_molecule = [
    ]  # record the geometry counts for each molecule
    for smiles in all_smiles:
        nodes, edges = to_graph(smiles, dataset)
        if len(edges) <= 0:
            continue
...
class MLP(object):
    ...
    def __call__(self, inputs):
        ...
        for W, b in zip(self.params["weights"], self.params["biases"]):
            hid = tf.matmul(
                acts, tf.nn.dropout(W, rate=1 - (self.dropout_keep_prob))) + b
            acts = tf.nn.relu(hid)
        last_hidden = hid
        return last_hidden
...
def overlapped_edge_features_to_dense(overlapped_edge_features,
                                      maximum_vertice_num):
    new_overlapped_edge_features = []
    for sparse_overlapped_edge_features in overlapped_edge_features:
        dense_overlapped_edge_features = np.zeros((maximum_vertice_num),
                                                  dtype=int)
...
def transition_freqs_edge_to_dense(transition_freqs,
                                   maximum_vertice_num,
                                   num_edge_types=3):
    new_freqs_sequence = []
    for step in transition_freqs:
...
def edge_type_masks_to_dense(edge_type_masks, maximum_vertice_num,
                             num_edge_types):
    new_edge_type_masks = []
