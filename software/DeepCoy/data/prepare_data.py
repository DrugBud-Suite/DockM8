...
def preprocess(raw_data, dataset, name, save_dir=''):
    ...
    fails = 0
    total = len(raw_data)
    for i, (smiles_1, smiles_2) in enumerate([
        (mol['smiles_1'], mol['smiles_2']) for mol in raw_data
    ]):
        (mol_in, mol_out), _, nodes_to_keep = align_smiles_by_MCS_it(
            smiles_1, smiles_2)
        if mol_out == []:
            fails += 1
            continue
...
if __name__ == "__main__":
    ...
    if args.get('--data_path') and args.get('--dataset_name'):
        ...
        data_paths = [
            'data_zinc_dekois_train.smi', 'data_zinc_dekois_valid.smi',
            'data_zinc_dude_train.smi', 'data_zinc_dude_valid.smi'
        ]
        names = [
            'zinc_dekois_train', 'zinc_dekois_valid', 'zinc_dude_train',
            'zinc_dude_valid'
        ]
        reverse = True
