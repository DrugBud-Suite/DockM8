    ...
    try:
        ...
        while repeat:
            ...
            for i, mol in enumerate(aligned_mols):
                ...
                # Update partial mol
                partial_mols[i] = Chem.rdmolops.DeleteSubstructs(
                    partial_mols[i], Chem.MolFromSmarts(res.smartsString))
        ...
        for i in range(total_atoms_aligned, mols[0].GetNumHeavyAtoms()):
            ...
            for j in range(total_atoms_aligned, mols[0].GetNumHeavyAtoms()):
                if aligned_mols[1].GetAtomWithIdx(
                        j).GetAtomicNum() == atom_type and j not in sub_idx:
                    sub_idx.append(j)
                    break
        # Perform alignment
        ...
        align_idx.extend(
            list(
                set(range(mols[0].GetNumHeavyAtoms())).difference(
                    set(align_idx))))
        aligned_mols[1] = Chem.rdmolops.RenumberAtoms(aligned_mols[1],
                                                      align_idx)
