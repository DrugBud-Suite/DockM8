...
def test_get_best_chain_edia(common_test_data):
    ...
    extracted_chain_path = get_best_chain_edia(pdb_code, output_dir)
    expected_chain_path = output_dir / f"{pdb_code}_A.pdb"
    assert extracted_chain_path.is_file()
    assert extracted_chain_path == expected_chain_path
    # Verify that the extracted chain file is not empty
    assert extracted_chain_path.stat().st_size > 0
    parser = PDBParser()
    structure = parser.get_structure('structure', extracted_chain_path)
