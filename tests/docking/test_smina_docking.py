...
def test_SminaDocking(test_data):
	...
	result_file = smina.dock_batch(batch_file=ligand_file,
	...
	result_df = smina.process_docking_result(result_file, n_poses=10)

	assert isinstance(result_df, pd.DataFrame)
	assert "Pose ID" in result_df.columns
	assert "SMINA_Affinity" in result_df.columns
	...
	docking_results = smina.dock(library=library_file,
									protein_file=protein_file,
									pocket_definition=pocket_definition,
									exhaustiveness=8,
									n_poses=10,
	...
	assert "SMINA_Affinity" in docking_results.columns
