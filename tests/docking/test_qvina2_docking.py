...
def test_qvina2_dock_batch(test_data):
    ...
    assert qvina2._temp_dir is not None
    assert qvina2._temp_dir.exists()
    assert (qvina2._temp_dir / "run_info.json").exists()

    # Test dock_batch method with a single ligand file
    result_file = qvina2.dock_batch(
    ...
    assert all(pid.startswith(id_ + "_QVINA2_") for pid, id_ in zip(output_df["Pose ID"], output_df["ID"], strict=False))

    if qvina2._temp_dir.exists():
        shutil.rmtree(qvina2._temp_dir, ignore_errors=True)

def test_qvina2_dock_full(test_data):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data
    ...
    assert "QVINA2_Affinity" in final_df.columns
    ...
    final_df["QVINA2_Affinity"] = pd.to_numeric(final_df["QVINA2_Affinity"], errors="coerce")
    for id_ in final_df["ID"].unique():
        ...
        assert all(compound_poses["QVINA2_Affinity"].diff().fillna(0) >= 0)

    # Verify temporary directory cleanup
    assert not qvina2._temp_dir.exists()

def test_qvina2_dock_failure(test_data, tmp_path):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data
    ...
    assert qvina2._temp_dir is not None
    assert qvina2._temp_dir.exists()
    initial_temp_dir = qvina2._temp_dir
    run_id = qvina2._run_id
    ...
    output_sdf = output_dir / "qvina2_docking_results_fail.sdf"
    final_output = qvina2.dock(
    ...
    recovery_dir = Path.home() / "dockm8_recovery" / f"failed_{qvina2.name}_{run_id}"
...
def test_qvina2_resume_dock(test_data):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data
    
    # Simulate a failed run by creating a temporary directory and run_info
    qvina2_resume = Qvina2Docking(software)
    initial_temp_dir = qvina2_resume._temp_dir
    ...
    params_to_save = {
        ...
        "exhaustiveness": 1,
        "n_poses": 10,
        "output_sdf": str(output_dir / "qvina2_docking_results_resumed.sdf"),
    }
    ...
    run_info.update({
        ...
        "run_id": qvina2_resume._run_id
    })
    with open(initial_temp_dir / "run_info.json", "w") as f:
        json.dump(run_info, f, indent=2)
    
    # Resume the docking run
    resumed_qvina2 = Qvina2Docking.resume_from_recovery(initial_temp_dir)
    assert resumed_qvina2 is not None
    assert resumed_qvina2._run_id == qvina2_resume._run_id
    
    final_output_resumed = resumed_qvina2.resume_dock(n_cpus=n_cpus)
    ...
    # Verify cleanup
    assert not resumed_qvina2._temp_dir.exists()
    assert not initial_temp_dir.exists()

def test_qvina2_resume_dock_no_run_info(test_data, tmp_path):
    # Attempt to resume from a directory without run_info.json
    recovery_dir = tmp_path / "invalid_recovery"
    recovery_dir.mkdir()
    resumed_qvina2 = Qvina2Docking.resume_from_recovery(recovery_dir)
    assert resumed_qvina2 is None

def test_qvina2_resume_dock_invalid_run_info(test_data, tmp_path):
