...
def test_FABindDocking(test_data):
	protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data

	fabind = FABindDocking(software)

	# Test the full docking process with the library
	output_sdf = output_dir / "fabind_docking_results.sdf"
	docking_results = fabind.dock(library=library_file,
	...
	assert len(docking_results) <= num_library_ligands
