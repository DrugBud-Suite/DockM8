...
def test_GninaDocking(test_data):
	...
	result_file = gnina.dock_batch(batch_file=ligand_file,
	...
	result_df = gnina.process_docking_result(result_file, n_poses=10)

	assert isinstance(result_df, pd.DataFrame)
	assert "Pose ID" in result_df.columns
	assert "CNN-Score" in result_df.columns
	...
	docking_results = gnina.dock(library=library_file,
									protein_file=protein_file,
									pocket_definition=pocket_definition,
									exhaustiveness=8,
									n_poses=10,
	...
	assert "CNN-Score" in docking_results.columns
