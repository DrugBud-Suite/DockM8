...
def test_PsovinaDocking(test_data):
	...
	assert result_file.is_dir()                     # PSOVINA returns a directory

	# Test process_docking_result method
	result_df = psovina.process_docking_result(result_file, n_poses=10)

	assert isinstance(result_df, pd.DataFrame)
	assert "Pose ID" in result_df.columns
	assert "PSOVINA_Affinity" in result_df.columns
	...
	output_sdf = output_dir / "psovina_docking_results.sdf"
	docking_results = psovina.dock(library=library_file,
									protein_file=protein_file,
									pocket_definition=pocket_definition,
									exhaustiveness=8,
									n_poses=10,
	...
	assert "PSOVINA_Affinity" in docking_results.columns
	...
	assert "PSOVINA_Affinity" in output_df.columns
