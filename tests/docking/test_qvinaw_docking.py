...
def test_qvinaw_dock_batch(test_data):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data

    qvinaw = QvinawDocking(software)
    assert qvinaw._temp_dir is not None
    assert qvinaw._temp_dir.exists()
    assert (qvinaw._temp_dir / "run_info.json").exists()

    # Test dock_batch method with a single ligand file
    result_file = qvinaw.dock_batch(
    ...
    assert "QVINAW_Affinity" in output_df.columns
    ...
    assert all(pid.startswith(id_ + "_QVINAW_") for pid, id_ in zip(output_df["Pose ID"], output_df["ID"], strict=False))

    if qvinaw._temp_dir.exists():
        shutil.rmtree(qvinaw._temp_dir, ignore_errors=True)

def test_qvinaw_dock_full(test_data):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data
    ...
    output_sdf = output_dir / "qvinaw_docking_results.sdf"
    final_output = qvinaw.dock(
    ...
    assert "QVINAW_Affinity" in final_df.columns
    ...
    final_df["QVINAW_Affinity"] = pd.to_numeric(final_df["QVINAW_Affinity"], errors="coerce")
    for id_ in final_df["ID"].unique():
        ...
        assert all(compound_poses["QVINAW_Affinity"].diff().fillna(0) >= 0)

    # Verify temporary directory cleanup
    assert not qvinaw._temp_dir.exists()

def test_qvinaw_dock_failure(test_data, tmp_path):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data
    ...
    assert qvinaw._temp_dir is not None
    assert qvinaw._temp_dir.exists()
    initial_temp_dir = qvinaw._temp_dir
    run_id = qvinaw._run_id
    ...
    output_sdf = output_dir / "qvinaw_docking_results_fail.sdf"
    final_output = qvinaw.dock(
    ...
    recovery_dir = Path.home() / "dockm8_recovery" / f"failed_{qvinaw.name}_{run_id}"
...
def test_qvinaw_resume_dock(test_data):
    protein_file, library_file, ligand_file, software, n_cpus, output_dir, pocket_definition = test_data
    
    # Simulate a failed run by creating a temporary directory and run_info
    qvinaw_resume = QvinawDocking(software)
    initial_temp_dir = qvinaw_resume._temp_dir
    ...
    params_to_save = {
        ...
        "exhaustiveness": 1,
        "n_poses": 10,
        "output_sdf": str(output_dir / "qvinaw_docking_results_resumed.sdf"),
    }
    ...
    run_info.update({
        ...
        "run_id": qvinaw_resume._run_id
    })
    with open(initial_temp_dir / "run_info.json", "w") as f:
        json.dump(run_info, f, indent=2)
    
    # Resume the docking run
    resumed_qvinaw = QvinawDocking.resume_from_recovery(initial_temp_dir)
    assert resumed_qvinaw is not None
    assert resumed_qvinaw._run_id == qvinaw_resume._run_id
    
    final_output_resumed = resumed_qvinaw.resume_dock(n_cpus=n_cpus)
    ...
    # Verify cleanup
    assert not resumed_qvinaw._temp_dir.exists()
    assert not initial_temp_dir.exists()

def test_qvinaw_resume_dock_no_run_info(test_data, tmp_path):
    # Attempt to resume from a directory without run_info.json
    recovery_dir = tmp_path / "invalid_recovery"
    recovery_dir.mkdir()
    resumed_qvinaw = QvinawDocking.resume_from_recovery(recovery_dir)
    assert resumed_qvinaw is None

def test_qvinaw_resume_dock_invalid_run_info(test_data, tmp_path):
