...
def test_QvinawDocking(test_data):
	...
	result_file = qvinaw.dock_batch(batch_file=ligand_file,
	...
	assert result_file.is_dir()                     # QVINAW returns a directory

	# Test process_docking_result method
	result_df = qvinaw.process_docking_result(result_file, n_poses=10)

	assert isinstance(result_df, pd.DataFrame)
	assert "Pose ID" in result_df.columns
	assert "QVINAW_Affinity" in result_df.columns
	...
	docking_results = qvinaw.dock(library=library_file,
									protein_file=protein_file,
									pocket_definition=pocket_definition,
									exhaustiveness=8,
									n_poses=10,
	...
	assert "QVINAW_Affinity" in docking_results.columns
