...
def test_PlantsDocking(test_data):
	...
	result_file = plants.dock_batch(batch_file=ligand_file,
	...
	result_df = plants.process_docking_result(result_file, n_poses=10)

	assert isinstance(result_df, pd.DataFrame)
	assert "Pose ID" in result_df.columns
	assert "CHEMPLP" in result_df.columns
	...
	docking_results = plants.dock(library=library_file,
									protein_file=protein_file,
									pocket_definition=pocket_definition,
									exhaustiveness=8,
									n_poses=10,
	...
	assert "CHEMPLP" in docking_results.columns
