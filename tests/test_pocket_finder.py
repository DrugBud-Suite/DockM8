...
def common_test_data():
    ...
    w_dir = base_path / 'test_files/1fvv_p_protoss'
    radius = 10
    return ligand, receptor, w_dir, radius

def test_reference_mode(common_test_data):
    """Test pocket finding in reference mode."""
    ligand, receptor, w_dir, radius = common_test_data
...
def test_dogsitescorer_mode(common_test_data):
    ...
    ligand, receptor, w_dir, radius = common_test_data
    w_dir = Path(__file__).parent.resolve()
    pocket_definition = pocket_finder('Dogsitescorer', receptor=receptor, w_dir=w_dir) 
