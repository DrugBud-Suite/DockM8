...
def common_test_data():
    ...
    input_sdf = Path(
        dockm8_path /
        "tests/test_files/docking_postprocessing/example_poses_1fvv.sdf")
    output_path = Path(
        dockm8_path /
        "tests/test_files/docking_postprocessing/example_poses_1fvv_postprocessed.sdf"
    )
    protein_file = Path(
        dockm8_path /
        "tests/test_files/docking_postprocessing/example_prepared_receptor_1fvv.pdb"
    )
    bust_poses = True
    strain_cutoff = 5
    clash_cutoff = 3
...
def test_docking_postprocessing(common_test_data, cleanup):
    ...
    result = docking_postprocessing(input_sdf, output_path, protein_file,
                                    bust_poses, strain_cutoff, clash_cutoff,
                                    n_cpus)
