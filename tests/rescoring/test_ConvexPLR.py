...
def test_ConvexPLR_rescoring(test_data):
    ...
    result = convexplr.rescore(sdf, n_cpus, protein_file=protein_file)

    assert isinstance(result, DataFrame)
    assert "Pose ID" in result.columns
    assert convexplr.column_name in result.columns
    assert len(result) > 0
    ...
    output_file = output_dir / f"{convexplr.column_name}_scores.csv"
