...
def test_AAScore_rescoring(test_data):
	...
	aascore = AAScore(software)

	result = aascore.rescore(clustered_sdf, n_cpus, software=software, protein_file=protein_file)

	assert isinstance(result, DataFrame)
	assert "Pose ID" in result.columns
	assert aascore.column_name in result.columns
	assert len(result) > 0
	...
	output_file = output_dir / f"{aascore.column_name}_scores.csv"
