...
def test_Vinardo_rescoring(test_data):
    ...
    result = vinardo.rescore(sdf, n_cpus, protein_file=protein_file)

    assert isinstance(result, DataFrame)
    assert "Pose ID" in result.columns
    assert vinardo.column_name in result.columns
    assert len(result) > 0
    ...
    output_file = output_dir / f"{vinardo.column_name}_scores.csv"
