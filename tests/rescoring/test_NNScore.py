...
def test_NNScore_rescoring(test_data):
    protein_file, software, sdf, n_cpus, output_dir = test_data

    nnscore = NNScore(software)

    result = nnscore.rescore(sdf, n_cpus, protein_file=protein_file)

    assert isinstance(result, DataFrame)
    assert "Pose ID" in result.columns
    assert nnscore.column_name in result.columns
    assert len(result) > 0
    ...
    output_file = output_dir / f"{nnscore.column_name}_scores.csv"
