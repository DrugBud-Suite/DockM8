...
def test_CnnAffinity_rescoring(test_data):
    protein_file, software, sdf, n_cpus, output_dir = test_data

    cnn_affinity = CnnAffinity(software)

    result = cnn_affinity.rescore(sdf, n_cpus, protein_file=protein_file)

    assert isinstance(result, DataFrame)
    assert "Pose ID" in result.columns
    assert cnn_affinity.column_name in result.columns
    assert len(result) > 0
    ...
    output_file = output_dir / f"{cnn_affinity.column_name}_scores.csv"
