...
def test_RFScoreVS_rescoring(test_data):
    ...
    result = rfscorevs.rescore(sdf, n_cpus, protein_file=protein_file)

    assert isinstance(result, DataFrame)
    assert "Pose ID" in result.columns
    assert rfscorevs.column_name in result.columns
    assert len(result) > 0
    ...
    output_file = output_dir / f"{rfscorevs.column_name}_scores.csv"
