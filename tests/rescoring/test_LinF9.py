...
def test_LinF9_rescoring(test_data):
	...
	linf9 = LinF9(software)

	result = linf9.rescore(clustered_sdf, n_cpus, software=software, protein_file=protein_file)

	assert isinstance(result, DataFrame)
	assert "Pose ID" in result.columns
	assert linf9.column_name in result.columns
	assert len(result) > 0
	...
	output_file = output_dir / f"{linf9.column_name}_scores.csv"
